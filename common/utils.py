...
def get_latest_trading_date():
    """获取最近 A 股交易日（<= 今日），非交易日则返回前一交易日。从 TradingCalendar 表读取。"""
    from .models import TradingCalendar

    today = date.today()
    try:
        obj = (
            TradingCalendar.objects.filter(date__lte=today, is_trading_day=True)
            .order_by('-date')
            .first()
        )
        if obj is not None:
...
def load_trading_calendar_for_year(year: int):
    """
    ...
    logger.info("交易日历加载完成: 年份=%s, 总条数=%s, 其中交易日=%s", year, updated, trading_count)
